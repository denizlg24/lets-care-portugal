...
import { ChevronLeft, ChevronRight, Eye, PenLine, Plus, Settings2, Trash2 } from "lucide-react";
...
function adminBlogsPageHref(page: number): string {
  return page <= 1 ? "/admin/blogs" : `/admin/blogs?page=${page}`;
}

export function BlogManager({ blogs, page, pages, total }: BlogManagerProps) {
  ...
  async function handleDelete(blog: BlogListItem) {
    ...
    try {
      const response = await fetchWithTimeout(`/api/admin/blogs/${blog.id}`, {
        method: "DELETE",
      });
      if (!response.ok) {
      ...
      {blogs.length === 0 ? (
        ...
        <>
          ...
          {pages > 1 ? (
            ...
            >
              <Link
                href={adminBlogsPageHref(Math.max(1, page - 1))}
              ...
              <Link
                href={adminBlogsPageHref(Math.min(pages, page + 1))}
